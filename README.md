...
지도가 기본 화면이고, **필요한 조작을 전부 지도에서** 할 수 있습니다.

- **3계층 지도** — 하늘 · 지상 · 지저를 탭으로 전환. 끌어서 이동, 휠 · 두 손가락 확대,
  더블클릭으로 전체 보기. 커서 위치의 게임 좌표가 지도 왼쪽 아래에 표시됩니다.
- **아이콘을 누르면 방향** — 보스를 고르면 추천 워프 3곳이 화살표 달린 선으로 그려지고,
  각 경로마다 `↗ 북동 1457m · 강하 321m` 처럼 **방위 · 수평 거리 · 고도 변화**가 나옵니다.
  워프 지점을 고르면 반대로 "여기서 가까운 미처치 보스"를 방향과 함께 보여줍니다.
- **지도에서 바로 체크** — 선택 패널에서 보스 처치 체크, 사당 · 조망대 해금 체크를 그 자리에서.
- **카테고리 토글** — 라이넬 · 히녹스 · 바위록 · 조망대 · 사당을 켜고 끄며, 각 버튼에 진행도(`12/34`)가 표시됩니다.
- **처치 완료 숨기기** · **이름으로 찾기** — 보스 · 사당 · 조망대 · 지역을 한글/영문으로 검색해 지도로 이동.
- **주소창 딥링크** — 지도를 움직이면 `?layer=Surface&x=-3378&y=30&z=5&sel=boss:lynel-020` 이
  주소에 반영됩니다. 링크를 공유하거나 새로고침해도 같은 화면이 뜹니다.
- **붉은 달** — 버튼 한 번으로 처치 기록만 일괄 초기화(워프 해금 상태는 유지)
- **목록 탭** — 190기를 추천 이동 시간 · 지역 · 고도 순으로 정렬해 훑어보기. 🗺 버튼으로 지도의 해당 위치로 이동.
- **한국어 표기** — 사당 152곳, 조망대 15곳, 보스 종류, 지역 190곳 모두 게임의 한국어 명칭을 사용합니다
- **오프라인 / 홈 화면 추가** — 서비스 워커가 앱 셸과 지도·데이터를 전부 캐시
