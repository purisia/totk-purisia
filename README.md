...
- **마커** — 뾰족한 끝이 실제 좌표를 가리키는 물방울 핀. 종류별 색과 흰 글리프
  ...
  흰 갈기 · 실버 라이넬, 블랙 히녹스, 야광 · 레어 바위록은 따로 구분해 표시합니다.
...
- **계층** — 하늘 · 지상 · 지저
- **적 강화 단계** — 게임은 쓰러뜨린 적이 쌓이면 배치된 개체를 같은 계열의 윗 단계로
  올립니다(라이넬 → 푸른 갈기 → 하얀 갈기 → 실버). 데이터에 든 것은 **처음 배치**라,
  진행이 된 세이브에서는 실제로 보이는 종류가 더 위입니다. 기본 / 강화 1 / 강화 2 / 최대
  중에서 고르면 이름 · 아이콘 · 필터 · 개수가 모두 그 단계로 바뀝니다.

  | 계열 | 사슬 |
  | --- | --- |
  | 라이넬 | 라이넬 → 푸른 갈기 → 하얀 갈기 → 실버 |
  | 히녹스 | 히녹스 → 블루 → 블랙 |
  | 바위록 | 바위록 → 야광 → 레어 |

  **강화되지 않는 64기**는 지도에서 <b>각진 핀</b>으로 구분되고, 카드·패널에 "강화 없음"
  배지가 붙습니다. 서랍의 "강화 안 되는 개체만" 으로 이들만 골라 볼 수도 있습니다.
  ...
  | 스탈 히녹스 | 20 |
  | 꽁꽁록 / 마그록 | 9 / 9 |
  | 퀘스트 연동(히녹스 · 바위록 · 마그록 · 꽁꽁록 각 1) | 4 |
- **보스 표시** — 8개 카테고리 토글(각 버튼에 진행도), **보스 전체 끄기/켜기** 한 번에,
  처치 완료 숨기기, 지도 전체 보기

  소재값이 큰 상위 변종은 아이콘 · 색 · 필터를 따로 씁니다.

  | 카테고리 | 수 | 표시 |
  | --- | --- | --- |
  | 라이넬 / **흰 갈기** / **실버** | 22 / **9** / **3** | 화살 / **왕관+화살** / **별+화살(은색 핀)** |
  | 히녹스 / **블랙** | 52 / **17** | 외눈 / **긴 뿔 + 째진 눈** |
  | 바위록 / **야광** / **레어** | 51 / **22** / **14** | 수정 / **빛나는 수정** / **반짝이는 수정** |

  사당(152)과 조망대(15)는 **끌 수 없습니다** — 추천 경로 계산의 기준이라 항상 표시되고,
  서랍에는 해금 진행도만 나옵니다.
...
| `data/scaling.json` | 적 강화 사슬 4계열 (라이넬 · 라이넬(기갑) · 히녹스 · 바위록) |
